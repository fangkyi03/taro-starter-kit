...
const baseUrl = "http://182.61.13.115:7041";
...
function tranData(data, path = '') {
    const str = Object.keys(data).map((e) => {
        control.add(e + '_' + path.split('/').join('').toLocaleUpperCase())
        return tranParent(e + '_' + path.split('/').join('').toLocaleUpperCase(), data[e], path)
        // if (control.has(e)) {
...
function getNetInfo(filterData, excludeData) {
    axios.get("http://182.61.13.115:7041/swagger-resources")
        .then(async (e) => {
...
getNetInfo(["app/firm", "trace/H5", "trace", 'admin', 'firm', 'insur']);